...
async function startWatcher(opts) {
    ...
    const hotReloader = opts.turbo ? await (async ()=>{
        ...
        return await createHotReloaderTurbopack(opts, serverFields, distDir, resetFetch, lockfile, opts.experimentalServerFastRefresh);
    })() : await (async ()=>{
    ...
    }, _path.default.join(distTypesDir, 'route-types.d.ts'), opts.nextConfig);
    // Write the entry file at {distDirRoot}/types/routes.d.ts for initial state
    const initialEntryFilePath = _path.default.join(opts.dir, opts.nextConfig.distDirRoot, 'types', 'routes.d.ts');
    await (0, _routetypesutils.writeRouteTypesEntryFile)(initialEntryFilePath, distTypesDir, {
         ...
         */ if (/[\\\\/]_middleware$/.test(pageName)) {
            ...
            try {
                ...
                if (enabledTypeScript) {
                    ...
                    const entryFilePath = _path.default.join(opts.dir, opts.nextConfig.distDirRoot, 'types', 'routes.d.ts');
                    await (0, _routetypesutils.writeRouteTypesEntryFile)(entryFilePath, distTypesDir, {
                        strictRouteTypes: Boolean(nextConfig.experimental.strictRouteTypes),
                        typedRoutes: Boolean(nextConfig.typedRoutes)
                    });
                }
