 ...
 */ async function generateDynamicRSCPayload(ctx, options) {
    ...
    if (!(options == null ? void 0 : options.skipPageRendering)) {
        ...
        await (0, _instantconfig.anySegmentNeedsInstantValidation)(loaderTree);
...
async function stagedRenderToReadableStreamWithoutCachesInDev(ctx, requestStore, getPayload, options) {
    ...
    const stageController = new _stagedrendering.StagedRenderingController(abortSignal);
    ...
    return await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        return _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, renderToReadableStream, rscPayload, clientModules, {
            ...options,
            environmentName
        });
    }, ()=>{
 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, devFallbackParams) {
    ...
    const shouldValidate = !isBypassingCachesInDev(initialRequestStore) && (initialRequestStore.isHmrRefresh === true || await (0, _instantconfig.anySegmentNeedsInstantValidation)(loaderTree));
    ...
    !isBypassingCachesInDev(initialRequestStore)) {
        ...
        if (shouldValidate) {
            ...
            _consoleasyncstorageexternal.consoleAsyncStorage.run({
                dim: true
            }, spawnStaticShellValidationInDev, accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, ctx, finalRequestStore, devFallbackParams, validationDebugChannelClient);
        }
...
async function finalRuntimeServerPrerender(ctx, getPayload, prerenderResumeDataCache, renderResumeDataCache, rootParams, headers, cookies, draftMode, onError, staleTimeIterable) {
    ...
    const finalStageController = new _stagedrendering.StagedRenderingController(finalServerController.signal);
    ...
    const result = await (0, _apprenderrenderutils.runInSequentialTasks)(async ()=>{
        ...
        (0, _staletime.finishStaleTimeTracking)(staleTimeIterable).then(()=>{
...
async function getRSCPayload(tree, ctx, is404) {
    ...
    const initialTree = await (0, _createflightrouterstatefromloadertree.createFlightRouterStateFromLoaderTree)(tree, getDynamicParamFromSegment, query);
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, createRequestStore, devFallbackParams) {
    ...
    const { basePath, buildManifest, ComponentMod: { createElement, renderToReadableStream: serverRenderToReadableStream }, crossOrigin, experimental, isBuildTimePrerendering = false, onInstrumentationRequestError, page, reactMaxHeadersLength, setReactDebugChannel, shouldWaitOnAllReady, subresourceIntegrityManifest, supportsDynamicResponse, cacheComponents } = renderOpts;
    ...
    const bootstrapScriptContent = process.env.__NEXT_DEV_SERVER ? `self.__next_r=${JSON.stringify(requestId)}` : undefined;
...
async function renderWithRestartOnCacheMissInDev(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    ;
    const initialStageController = new _stagedrendering.StagedRenderingController(initialDataController.signal, initialAbandonController);
    ...
    const initialStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        ...
        const streamPair = _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, ComponentMod.renderToReadableStream, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    const finalStreamResult = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
        finalStageController.advanceStage(_stagedrendering.RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const streamPair = _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, ComponentMod.renderToReadableStream, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
 ...
 */ async function spawnStaticShellValidationInDevImpl(accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, ctx, requestStore, fallbackRouteParams, debugChannelClient) {
    ...
    const needsInstantValidation = await (0, _instantconfig.anySegmentNeedsInstantValidation)(loaderTree);
    ...
    await warmupClientModulesForStagedValidationInDev(// if we're going to be validating prefetches, we'll be rendering some segments in the dynamic stage.
    ...
    needsInstantValidation ? 'validation-client' : 'prerender-client', needsInstantValidation ? dynamicChunks : runtimeChunks, dynamicChunks, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx);
    ...
    if (needsInstantValidation) {
        const instantConfigsResult = await validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, ctx, hmrRefreshHash);
        if (instantConfigsResult.length > 0) {
            return logMessagesAndSendErrorsToBrowser(instantConfigsResult, ctx);
        }
    }
}
async function warmupClientModulesForStagedValidationInDev(storeType, partialServerChunks, allServerChunks, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx) {
...
async function validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, ctx, hmrRefreshHash) {
    const debug = process.env.NEXT_PRIVATE_DEBUG_VALIDATION === '1' ? console.log : undefined;
    const { findNavigationsToValidate, collectStagedSegmentData } = ctx.componentMod.InstantValidation;
    debug == null ? void 0 : debug('\nStarting instant validation...');
    let init;
    // We throw for invalid configurations, so errors should be surfaced.
    try {
        init = await findNavigationsToValidate(ctx.componentMod.routeModule.userland.loaderTree, ctx.getDynamicParamFromSegment, ctx.query);
    } catch (err) {
        debug == null ? void 0 : debug('Error while planning validations.');
        return [
            err
        ];
    }
    const { tree: validationRouteTree, treeNodes, navigationParents, segmentsWithInstantConfigs } = init;
    const hasRuntimePrefetch = segmentsWithInstantConfigs.some((segmentPath)=>{
        ...
        return instantConfig && typeof instantConfig === 'object' ? instantConfig.prefetch === 'runtime' : false;
    });
    ...
    const getFilepathForSegment = (segmentPath)=>{
        var _treeNodes_get_module, _treeNodes_get;
        return (_treeNodes_get = treeNodes.get(segmentPath)) == null ? void 0 : (_treeNodes_get_module = _treeNodes_get.module) == null ? void 0 : _treeNodes_get_module.conventionPath;
    };
    for (const navigationParent of navigationParents){
        // TODO(instant-validation): report which segment had errors
        debug == null ? void 0 : debug(`-------------------------------\n` + `Validating navigation\n` + `  from '${navigationParent}/*' (${getFilepathForSegment(navigationParent) ?? '<no file>'})\n` + `  to   '${_workasyncstorageexternal.workAsyncStorage.getStore().route}'`);
        const initialResults = await validateInstantConfigNavigation(initialRscPayload, cache, startTime, stageEndTimes, rootParams, ctx, hmrRefreshHash, validationRouteTree, navigationParent, false // use static stage for static segments
        );
        if (initialResults.errors.length === 0) {
            debug == null ? void 0 : debug(`  ✅ Validation successful`);
        }
        if (initialResults.errors.length > 0) {
            if (initialResults.dynamicHoleKind !== _dynamicrendering.DynamicHoleKind.Dynamic) {
                debug == null ? void 0 : debug('  Retrying to gather more info...');
                const runtimeResults = await validateInstantConfigNavigation(initialRscPayload, cache, startTime, stageEndTimes, rootParams, ctx, hmrRefreshHash, validationRouteTree, navigationParent, true // use runtime stage for static segments instead
                );
                if (runtimeResults.errors.length > 0) {
                    // The errors remained in the runtime stage, so they were caused by a dynamic access.
                    debug == null ? void 0 : debug(`  ❌ Failed after runtime retry (${runtimeResults.errors.length} errors)`);
            ...
            debug == null ? void 0 : debug(`  ❌ Failed (${initialResults.errors.length} errors)`);
...
async function validateInstantConfigNavigation(initialRscPayload, cache, startTime, stageEndTimes, rootParams, ctx, hmrRefreshHash, routeTree, navigationParent, useRuntimeStageForPartialSegments) {
    const { implicitTags, nonce, workStore } = ctx;
    const isDebugChannelEnabled = !!ctx.renderOpts.setReactDebugChannel;
    const { createCombinedPayload, createCombinedPayloadStream } = ctx.componentMod.InstantValidation;
    const clientDynamicTracking = (0, _dynamicrendering.createDynamicTrackingState)(false //isDebugDynamicAccesses
    );
    const clientReactController = new AbortController();
    const clientRenderController = new AbortController();
    const preinitScripts = ()=>{};
    const { ServerInsertedHTMLProvider } = (0, _serverinsertedhtml.createServerInsertedHTML)();
    const dynamicValidation = (0, _dynamicrendering.createInstantValidationState)();
    ...
    const clientReferenceManifest = (0, _manifestssingleton.getClientReferenceManifest)();
    const usedSegmentKinds = new Set();
    const { stream: serverStream, debugStream } = await createCombinedPayloadStream((extraChunksReleaseSignal)=>createCombinedPayload(initialRscPayload, cache, routeTree, navigationParent, extraChunksReleaseSignal, boundaryState, clientReferenceManifest, stageEndTimes, useRuntimeStageForPartialSegments, usedSegmentKinds), clientReactController.signal, clientReferenceManifest, startTime, isDebugChannelEnabled);
    ...
    const dynamicHoleKind = usedSegmentKinds.has(_stagedrendering.RenderStage.Static) ? _dynamicrendering.DynamicHoleKind.Runtime : _dynamicrendering.DynamicHoleKind.Dynamic;
    const prerender = require('react-dom/static').prerender;
    try {
        let { prelude: unprocessedPrelude } = await (0, _apprenderrenderutils.runInSequentialTasks)(()=>{
            const pendingFinalClientResult = _workunitasyncstorageexternal.workUnitAsyncStorage.run(finalClientPrerenderStore, prerender, // eslint-disable-next-line @next/internal/no-ambiguous-jsx -- React Client
            /*#__PURE__*/ (0, _jsxruntime.jsx)(App, {
                ...
                onError: (err, errorInfo)=>{
                    if ((0, _dynamicrendering.isPrerenderInterruptedError)(err) || clientReactController.signal.aborted) {
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            (0, _dynamicrendering.trackDynamicHoleInNavigation)(workStore, componentStack, dynamicValidation, clientDynamicTracking, dynamicHoleKind, boundaryState);
                        }
                        return;
                    } else if (!clientReactController.signal.aborted) {
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            (0, _dynamicrendering.trackThrownErrorInNavigation)(dynamicValidation, err, componentStack);
                        }
                    }
                    if ((0, _reactlargeshellerror.isReactLargeShellError)(err)) {
                        // TODO: Aggregate
        ...
        const { preludeIsEmpty } = await (0, _apprenderprerenderutils.processPrelude)(unprocessedPrelude);
        const reasons = (0, _dynamicrendering.getNavigationDisallowedDynamicReasons)(workStore, preludeIsEmpty ? _dynamicrendering.PreludeState.Empty : _dynamicrendering.PreludeState.Full, dynamicValidation, boundaryState);
        return {
            dynamicHoleKind,
            errors: reasons
        };
        ...
        // Even if the root errors we still want to report any cache components errors
        // that were discovered before the root errored.
        let errors = (0, _dynamicrendering.getNavigationDisallowedDynamicReasons)(workStore, _dynamicrendering.PreludeState.Errored, dynamicValidation, boundaryState);
        if (process.env.NEXT_DEBUG_BUILD || process.env.__NEXT_VERBOSE_LOGGING) {
            errors.unshift('During dynamic validation the root of the page errored. The next logged error is the thrown value. It may be a duplicate of errors reported during the normal development mode render.', thrownValue);
        }
        return {
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            let prerenderIsPending = true;
            const reactServerResult = reactServerPrerenderResult = await (0, _apprenderprerenderutils.createReactServerPrerenderResult)((0, _apprenderrenderutils.runInSequentialTasks)(async ()=>{
                ...
                // storage context.
                await (0, _varyparams.finishAccumulatingVaryParams)(varyParamsAccumulator);
