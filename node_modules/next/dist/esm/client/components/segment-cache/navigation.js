...
import { EntryStatus, readRouteCacheEntry, deprecated_requestOptimisticRouteCacheEntry, convertRootFlightRouterStateToRouteTree } from './cache';
 ...
 */ export function navigate(state, url, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, nextUrl, freshnessPolicy, shouldScroll, navigateType) {
    const now = Date.now();
    const href = url.href;
...
export function navigateToKnownRoute(now, state, url, canonicalUrl, navigationSeed, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll, navigateType, debugInfo, // The route cache entry used for this navigation, if it came from route
...
routeCacheEntry) {
    ...
    if (task !== null) {
        if (freshnessPolicy !== FreshnessPolicy.Gesture) {
            spawnDynamicRequests(task, url, nextUrl, freshnessPolicy, accumulation, routeCacheEntry);
        }
...
async function navigateToUnknownRoute(now, state, url, currentUrl, currentRenderedSearch, nextUrl, currentCacheNode, currentFlightRouterState, freshnessPolicy, shouldScroll, navigateType) {
    ...
    if (metadataVaryPath !== null) {
        discoverKnownRoute(now, url.pathname, null, navigationSeed.routeTree, metadataVaryPath, couldBeIntercepted, createHrefFromUrl(canonicalUrl), prerendered, false // hasDynamicRewrite - not a retry, rewrite detection happens during traversal
        );
    }
    return navigateToKnownRoute(now, state, url, createHrefFromUrl(canonicalUrl), navigationSeed, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll, navigateType, debugInfo, // Unknown route navigations don't use route prediction - the route tree
...
export function completeSoftNavigation(oldState, url, referringNextUrl, tree, cache, renderedSearch, canonicalUrl, navigateType, shouldScroll, scrollableSegments, collectedDebugInfo) {
