...
import { NEXT_DID_POSTPONE_HEADER, NEXT_INSTANT_PREFETCH_HEADER, NEXT_IS_PRERENDER_HEADER, NEXT_ROUTER_PREFETCH_HEADER, NEXT_ROUTER_SEGMENT_PREFETCH_HEADER, NEXT_ROUTER_STALE_TIME_HEADER, NEXT_ROUTER_STATE_TREE_HEADER, NEXT_URL, RSC_CONTENT_TYPE_HEADER, RSC_HEADER } from '../app-router-headers';
...
import { DYNAMIC_STALETIME_MS, STATIC_STALETIME_MS } from '../router-reducer/reducers/navigate-reducer';
...
export function attemptToFulfillDynamicSegmentFromBFCache(now, segment, tree) {
    ...
    const bfcacheEntry = readFromBFCacheDuringRegularNavigation(adjustedCurrentTime, varyPath);
...
export function fulfillRouteCacheEntry(now, entry, tree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled) {
...
export function writeRouteIntoCache(now, pathname, tree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled) {
    const pendingEntry = createDetachedRouteCacheEntry();
...
export async function fetchRouteOnCacheMiss(entry, task, key) {
    ...
    try {
        ...
        if (routeIsPPREnabled) {
            ...
            discoverKnownRoute(Date.now(), pathname, entry, routeTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, false // hasDynamicRewrite
            );
        } else {
            ...
            FetchStrategy.LoadingBoundary, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, headVaryParams, pathname);
        }
...
export async function fetchSegmentOnCacheMiss(route, segmentCacheEntry, routeKey, tree) {
    ...
    try {
        ...
        if (process.env.__NEXT_VARY_PARAMS && varyParams !== null) {
            ...
            const fulfilledVaryPath = getFulfilledSegmentVaryPath(tree.varyPath, varyParams);
            const isRevalidation = false;
...
export async function fetchSegmentPrefetchesUsingDynamicRequest(task, route, fetchStrategy, dynamicRequestTree, spawnedEntries) {
    ...
    try {
        ...
        if (fetchStrategy === FetchStrategy.PPRRuntime && !response.headers.get(NEXT_IS_PRERENDER_HEADER)) {
...
function writeDynamicTreeResponseIntoCache(now, task, fetchStrategy, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, headVaryParams, originalPathname) {
    ...
    const fulfilledEntry = discoverKnownRoute(now, originalPathname, entry, routeTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, false // hasDynamicRewrite
    );
