...
export function serverPatchReducer(state, action) {
    ...
    const now = Date.now();
    return navigateToKnownRoute(now, state, retryUrl, retryCanonicalUrl, retrySeed, currentUrl, currentRenderedSearch, state.cache, state.tree, FreshnessPolicy.RefreshAll, retryNextUrl, shouldScroll, navigateType, null, // Server patch (retry) navigations don't use route prediction. This is
