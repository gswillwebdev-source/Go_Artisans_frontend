...
import { getRuntimeStage, makeDevtoolsIOAwarePromise, makeHangingPromise } from '../dynamic-rendering-utils';
...
export async function cache(kind, id, boundArgsLength, originalFn, args) {
    ...
    if (renderResumeDataCache) {
        ...
        if (cachedResult !== undefined) {
            let existingResult = await cachedResult;
            ...
            if (existingResult !== undefined) {
                ...
                const implicitTags = (workUnitStore == null ? void 0 : (_workUnitStore_implicitTags = workUnitStore.implicitTags) == null ? void 0 : _workUnitStore_implicitTags.tags) ?? [];
                if (existingResult.entry.tags.some((tag)=>isRecentlyRevalidatedTag(tag, workStore)) || implicitTags.some((tag)=>isRecentlyRevalidatedTag(tag, workStore))) {
            ...
            if (workUnitStore !== undefined && existingResult !== undefined) {
                if (existingResult.entry.revalidate === 0 || existingResult.entry.expire < DYNAMIC_EXPIRE) {
                    switch(workUnitStore.type){
                        case 'prerender':
                            ...
                            if (existingResult.entry.revalidate === 0) {
                                ...
                                debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from static shell due to short expire value:', existingResult.entry.expire);
                            }
                            if (cacheSignal) {
                                cacheSignal.endRead();
                            }
                        ...
                        case 'request':
                            {
                                if (process.env.NODE_ENV === 'development') {
                                    if (existingResult.entry.revalidate === 0 && existingResult.hasExplicitRevalidate === false) {
                                        throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheZeroRevalidateErrorMessage), "__NEXT_ERROR_CODE", {
                                            value: "E1000",
                                            enumerable: false,
                                            configurable: true
                                    ...
                                    if (existingResult.entry.expire < DYNAMIC_EXPIRE && existingResult.hasExplicitExpire === false) {
                                        throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheShortExpireErrorMessage), "__NEXT_ERROR_CODE", {
                                            value: "E1009",
                                            enumerable: false,
                                            configurable: true
                ...
                if (existingResult.entry.stale < RUNTIME_PREFETCH_DYNAMIC_STALE) {
                    switch(workUnitStore.type){
                        case 'prerender-runtime':
                            ...
                            debug == null ? void 0 : debug('omitting entry', serializedCacheKey, 'from runtime shell due to short stale value:', existingResult.entry.stale);
                            if (cacheSignal) {
                                cacheSignal.endRead();
                            }
                            return makeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"');
    ...
    if (stream === undefined) {
        ...
        if (cacheHandler && !shouldForceRevalidate(workStore, workUnitStore)) {
            ...
            entry = await cacheHandler.get(serializedCacheKey, (workUnitStore == null ? void 0 : (_workUnitStore_implicitTags1 = workUnitStore.implicitTags) == null ? void 0 : _workUnitStore_implicitTags1.tags) ?? []);
        }
        if (entry) {
            var _workUnitStore_implicitTags2;
            const implicitTags = (workUnitStore == null ? void 0 : (_workUnitStore_implicitTags2 = workUnitStore.implicitTags) == null ? void 0 : _workUnitStore_implicitTags2.tags) ?? [];
        ...
        if (entry === undefined || currentTime > entry.timestamp + entry.expire * 1000 || workStore.isStaticGeneration && currentTime > entry.timestamp + entry.revalidate * 1000) {
            ...
            if (!workStore.isDraftMode) {
                ...
                if (prerenderResumeDataCache) {
                    ...
                    prerenderResumeDataCache.cache.set(serializedCacheKey, getNthCacheResult(split, 1));
                } else {
                    savedCacheResult = pendingCacheResult;
                }
                if (cacheHandler) {
                    const promise = cacheHandler.set(serializedCacheKey, savedCacheResult.then((r)=>r.entry));
