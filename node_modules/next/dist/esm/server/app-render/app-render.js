...
import { processPrelude, ReactServerResult, createReactServerPrerenderResult, createReactServerPrerenderResultFromRender } from './app-render-prerender-utils';
...
import { isPageAllowedToBlock, anySegmentNeedsInstantValidation } from './instant-validation/instant-config';
...
async function stagedRenderToReadableStreamWithoutCachesInDev(ctx, requestStore, getPayload, options) {
    ...
    return await runInSequentialTasks(()=>{
        ...
        return workUnitAsyncStorage.run(requestStore, renderToReadableStream, rscPayload, clientModules, {
            ...options,
            environmentName
        });
    }, ()=>{
 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, devFallbackParams) {
    ...
    const shouldValidate = !isBypassingCachesInDev(initialRequestStore) && (initialRequestStore.isHmrRefresh === true || await anySegmentNeedsInstantValidation(loaderTree));
...
async function finalRuntimeServerPrerender(ctx, getPayload, prerenderResumeDataCache, renderResumeDataCache, rootParams, headers, cookies, draftMode, onError, staleTimeIterable) {
    ...
    const finalStageController = new StagedRenderingController(finalServerController.signal);
...
async function getRSCPayload(tree, ctx, is404) {
    ...
    const initialTree = await createFlightRouterStateFromLoaderTree(tree, getDynamicParamFromSegment, query);
    const serveStreamingMetadata = !!ctx.renderOpts.serveStreamingMetadata;
...
async function renderWithRestartOnCacheMissInDev(ctx, initialRequestStore, createRequestStore, getPayload, onError) {
    ...
    ;
    const initialStageController = new StagedRenderingController(initialDataController.signal, initialAbandonController);
    ...
    const initialStreamResult = await runInSequentialTasks(()=>{
        ...
        const streamPair = workUnitAsyncStorage.run(requestStore, ComponentMod.renderToReadableStream, initialRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
    ...
    const finalStreamResult = await runInSequentialTasks(()=>{
        finalStageController.advanceStage(RenderStage.EarlyStatic);
        startTime = performance.now() + performance.timeOrigin;
        const streamPair = workUnitAsyncStorage.run(requestStore, ComponentMod.renderToReadableStream, finalRscPayload, clientModules, {
            onError,
            environmentName,
            startTime,
            filterStackFrame,
 ...
 */ async function spawnStaticShellValidationInDevImpl(accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, ctx, requestStore, fallbackRouteParams, debugChannelClient) {
    ...
    const needsInstantValidation = await anySegmentNeedsInstantValidation(loaderTree);
...
async function validateInstantConfigNavigation(initialRscPayload, cache, startTime, stageEndTimes, rootParams, ctx, hmrRefreshHash, routeTree, navigationParent, useRuntimeStageForPartialSegments) {
    ...
    const dynamicHoleKind = usedSegmentKinds.has(RenderStage.Static) ? DynamicHoleKind.Runtime : DynamicHoleKind.Dynamic;
    const prerender = require('react-dom/static').prerender;
    try {
        let { prelude: unprocessedPrelude } = await runInSequentialTasks(()=>{
            const pendingFinalClientResult = workUnitAsyncStorage.run(finalClientPrerenderStore, prerender, // eslint-disable-next-line @next/internal/no-ambiguous-jsx -- React Client
            /*#__PURE__*/ _jsx(App, {
                ...
                onError: (err, errorInfo)=>{
                    if (isPrerenderInterruptedError(err) || clientReactController.signal.aborted) {
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            trackDynamicHoleInNavigation(workStore, componentStack, dynamicValidation, clientDynamicTracking, dynamicHoleKind, boundaryState);
                        }
                        return;
                    } else if (!clientReactController.signal.aborted) {
                        const componentStack = errorInfo.componentStack;
                        if (typeof componentStack === 'string') {
                            trackThrownErrorInNavigation(dynamicValidation, err, componentStack);
                        }
                    }
                    if (isReactLargeShellError(err)) {
                        // TODO: Aggregate
        ...
        const { preludeIsEmpty } = await processPrelude(unprocessedPrelude);
        const reasons = getNavigationDisallowedDynamicReasons(workStore, preludeIsEmpty ? PreludeState.Empty : PreludeState.Full, dynamicValidation, boundaryState);
        return {
            dynamicHoleKind,
            errors: reasons
        };
        ...
        // Even if the root errors we still want to report any cache components errors
        // that were discovered before the root errored.
        let errors = getNavigationDisallowedDynamicReasons(workStore, PreludeState.Errored, dynamicValidation, boundaryState);
        if (process.env.NEXT_DEBUG_BUILD || process.env.__NEXT_VERBOSE_LOGGING) {
            errors.unshift('During dynamic validation the root of the page errored. The next logged error is the thrown value. It may be a duplicate of errors reported during the normal development mode render.', thrownValue);
        }
        return {
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            let prerenderIsPending = true;
            const reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResult(runInSequentialTasks(async ()=>{
