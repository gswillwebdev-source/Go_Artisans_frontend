...
export function createServerParamsForMetadata(underlyingParams, workStore) {
    const metadataVaryParamsAccumulator = getMetadataVaryParamsAccumulator();
    return createServerParamsForServerSegment(underlyingParams, workStore, metadataVaryParamsAccumulator, metadataIsRuntimePrefetchable);
}
// routes always runs in RSC context so it is equivalent to a Server Page Component
export function createServerParamsForRoute(underlyingParams, workStore, varyParamsAccumulator = null) {
    const workUnitStore = workUnitAsyncStorage.getStore();
    if (workUnitStore) {
        switch(workUnitStore.type){
            case 'prerender':
...
export function createServerParamsForServerSegment(underlyingParams, workStore, varyParamsAccumulator, isRuntimePrefetchable) {
    const workUnitStore = workUnitAsyncStorage.getStore();
    if (workUnitStore) {
        switch(workUnitStore.type){
            case 'prerender':
...
function createStaticPrerenderParams(underlyingParams, workStore, prerenderStore, varyParamsAccumulator) {
    const underlyingParamsWithVarying = varyParamsAccumulator !== null ? createVaryingParams(varyParamsAccumulator, underlyingParams) : underlyingParams;
