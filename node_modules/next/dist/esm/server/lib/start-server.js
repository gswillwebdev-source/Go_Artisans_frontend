...
export async function getRequestHandlers({ dir, port, isDev, onDevServerCleanup, server, hostname, minimalMode, keepAliveTimeout, experimentalHttpsServer, experimentalServerFastRefresh, quiet }) {
    return initialize({
        dir,
        port,
        hostname,
...
export async function startServer(serverOptions) {
    const { dir, isDev, hostname, minimalMode, allowRetry, keepAliveTimeout, selfSignedCertificate, experimentalServerFastRefresh } = serverOptions;
    let { port } = serverOptions;
    process.title = `next-server (v${"16.2.0-canary.53"})`;
    let handlersReady = ()=>{};
    let handlersError = ()=>{};
