...
export default function JobDetailsPage() {
    ...
    const { t } = useLanguage()
    const { canApplyToJobs, isFree, isPro, monthlyAppCount, FREE_APPLY_LIMIT, PRO_APPLY_LIMIT } = useSubscription()
    ...
    const handleApply = async () => {
        ...
        if (!canApplyToJobs()) {
            ...
            setError(`You've reached your ${limit} applications/month limit. ${isFree ? 'Upgrade to Pro for 35 applications/month.' : 'Upgrade to Premium for unlimited.'}`)
            return
        }
                        ...
                        <div>
                            ...
                            {(isFree || isPro) && (
                                <p className="text-center text-sm text-gray-500 mt-2">
                                    {monthlyAppCount}/{isFree ? FREE_APPLY_LIMIT : PRO_APPLY_LIMIT} applications used this month
                                    {isFree && monthlyAppCount >= FREE_APPLY_LIMIT && (
                                        <> · <Link href="/pricing" className="text-indigo-600 underline">Upgrade to Pro (35/month)</Link></>
                                    )}
                                    {isPro && monthlyAppCount >= PRO_APPLY_LIMIT && (
                                        <> · <Link href="/pricing" className="text-indigo-600 underline">Upgrade to Premium for unlimited</Link></>
                                    )}
                                </p>
                            )}
                        </div>
