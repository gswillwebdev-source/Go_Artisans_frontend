...
export default function BrowseWorkersPage() {
    ...
    const fetchWorkers = async (searchFilters = filters, nextPage = 1, append = false) => {
        try {
            ...
            let query = supabase
                .from('users')
                .select('id,first_name,last_name,email,phone_number,job_title,location,bio,years_experience,profile_picture,rating,is_active,referral_count,user_subscriptions(plan_id,status),verification_badges!verification_badges_user_id_fkey(status,badge_type)')
                .eq('user_type', 'worker')
                .eq('is_active', true)
            ...
            // Sort: Pro/Premium subscribers first, then free users
            const isPlanPro = (w) => {
                const sub = Array.isArray(w.user_subscriptions) ? w.user_subscriptions[0] : w.user_subscriptions
                if (!sub) return 0
                const tier = sub.plan_id?.split('_')[1] || 'free'
                if (tier === 'premium') return 2
                if (tier === 'pro') return 1
                return 0
            }

            const safeWorkers = (workersData || []).sort((a, b) => isPlanPro(b) - isPlanPro(a))
