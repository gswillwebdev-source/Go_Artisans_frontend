...
export default function WorkerProfilePage() {
    const { t } = useLanguage()
    const { isFree, isPro, isPremium, isVerified, badge, FREE_PORTFOLIO_LIMIT, PRO_PORTFOLIO_LIMIT } = useSubscription()
    const portfolioLimit = isFree ? FREE_PORTFOLIO_LIMIT : isPro ? PRO_PORTFOLIO_LIMIT : Infinity
    ...
    const [showPortfolioUpgradeModal, setShowPortfolioUpgradeModal] = useState(false)
    ...
    useEffect(() => {
        ...
        async function fetchProfile() {
            try {
                ...
                // Fetch referral count (non-blocking)
                supabase.auth.getSession().then(({ data: { session } }) => {
                    if (session?.access_token) {
                        fetch('/api/referrals', { headers: { Authorization: `Bearer ${session.access_token}` } })
                            .then(r => r.ok ? r.json() : null)
                            .then(data => { if (data?.count != null) setReferralCount(data.count) })
                            .catch(() => { })
                    }
                })
    ...
    useEffect(() => {
        ...
        async function fetchFollowCounts() {
            ...
            try {
                ...
                requestTimeoutId = setTimeout(() => controller.abort(), 15000)
    ...
    const handlePortfolioChange = (e) => {
        ...
        files.forEach(async (file) => {
            setFormData(prev => {
                if (prev.portfolio.length >= limit) {
                    setShowPortfolioUpgradeModal(true)
                    return prev
                }
                return prev // placeholder; actual update happens after compress
            })

            // Re-check synchronously before compressing (formData.portfolio is captured in closure)
            if (formData.portfolio.length >= limit) {
                setShowPortfolioUpgradeModal(true)
                return
            }
            ...
            setFormData(prev => {
                ...
                return { ...prev, portfolio: [...prev.portfolio, dataUrl] }
            })
    ...
    return (
        <div className="profile-page">
            <UpgradeModal
                isOpen={showPortfolioUpgradeModal}
                onClose={() => setShowPortfolioUpgradeModal(false)}
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <PlanBadge
                                            planTier={isPremium ? 'premium' : isPro ? 'pro' : 'free'}
                                    ...
                                    {getReferralTier(referralCount) && (
                                        <div className="mt-2">
                                            <ReferralBadge count={referralCount} />
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {profile.user_type === 'worker' && (
                            <>
                                ...
                                {/* Pro Star Badge — shown automatically for all Pro users */}
                                {isPro && (
                                    <div className="profile-section border border-yellow-100 bg-yellow-50/40">
                                        <h3 className="profile-title text-xl font-semibold mb-4 flex items-center gap-2">
                                            ⭐ Pro Member Badge
                                        </h3>
                                        <div className="flex items-center gap-3">
                                            <span className="inline-flex items-center gap-2 bg-yellow-400 text-yellow-900 px-4 py-2 rounded-full font-bold text-sm shadow-sm">
                                                ⭐ Pro Member
                                            </span>
                                            <p className="text-sm text-gray-600">Your Pro star badge is visible on your public profile and in search results.</p>
                                        </div>
                                    </div>
                                )}

                                {/* Who Viewed My Profile — Pro users only */}
                                {isPro && (
                                    <div className="profile-section border border-indigo-100 bg-indigo-50/30">
                                        <h3 className="profile-title text-xl font-semibold mb-4 flex items-center gap-2">
                                            👁️ Who Viewed My Profile
                                        </h3>
                                        <p className="text-sm text-gray-600 mb-4">See who has visited your profile in the last 30 days.</p>
                                        <a
                                            href="/profile-views"
                                            className="inline-flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-indigo-700 transition text-sm"
                                        >
                                ...
                                {/* Verification Section — Pro users only */}
                                {isPro && (
                                    <div className="profile-section border border-blue-100 bg-blue-50/40">
                                        <h3 className="profile-title text-xl font-semibold mb-4 flex items-center gap-2">
                                            {isVerified
                                        ...
                                        {isVerified ? (
                                            <div className="flex items-center gap-3">
                                                <span className="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full font-semibold text-sm">
                                                    <VerifiedBadge size={16} />
                                                </span>
                                                <p className="text-sm text-gray-600">Your identity has been verified. A blue badge is shown on your public profile.</p>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-yellow-700">Our team is reviewing your documents. You'll receive an email within 24 hours.</p>
                                                </div>
                                            </div>
                                        ) : (
                                            <div>
                                                <p className="text-sm text-gray-600 mb-4">Get a blue verification badge to build trust with clients. You'll need to upload a selfie and a valid ID.</p>
                                                <a
                                                    href="/notifications?tab=verification"
                                                    className="inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-blue-700 transition text-sm"
                                                >
                                ...
                                <div className="profile-section">
                                    ...
                                    <div className="profile-grid-4 mb-8">
                                        {/* Saved Jobs */}
                                        <Link
                                            href={isFree ? '/pricing' : '/saved-jobs'}
                                            className="profile-stat-card border-teal-200 bg-teal-50 hover:bg-teal-100 transition-colors cursor-pointer block no-underline"
                                        >
                                            ...
                                            {isFree ? (
                                                <p className="text-xs text-amber-600 mt-3 font-semibold">⭐ Pro / Premium only →</p>
                                            ) : (
                                                <p className="text-xs text-teal-600 mt-3 font-semibold">{t('viewAllSavedJobs')} →</p>
                                            )}
                                        </Link>
                        ...
                        <div className="profile-section">
                            ...
                            {portfolioLimit !== Infinity && (
                                <p className="text-sm mb-3">
                                    <span className={`font-semibold ${formData.portfolio.length >= portfolioLimit ? 'text-red-600' : 'text-gray-600'}`}>
                                        {formData.portfolio.length}/{portfolioLimit} portfolio items used
                                    </span>
                                    {formData.portfolio.length >= portfolioLimit && (
                                        <> · <button type="button" onClick={() => setShowPortfolioUpgradeModal(true)} className="text-indigo-600 underline">Upgrade for unlimited</button></>
                                    )}
                                </p>
                            )}
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    disabled={formData.portfolio.length >= portfolioLimit}
                                    className={`w-full px-4 py-2 border border-gray-300 rounded-lg cursor-pointer ${formData.portfolio.length >= portfolioLimit ? 'opacity-50 cursor-not-allowed' : ''}`}
                                />
