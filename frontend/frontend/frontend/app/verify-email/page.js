...
export default function VerifyEmailPage() {
    ...
    useEffect(() => {
        ...
        const checkUser = async () => {
            // Use getUser() which hits the Supabase server directly — never stale
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Force a server-side token refresh so email_confirmed_at reflects the latest state
            const { data: refreshData, error: refreshError } = await supabase.auth.refreshSession()

            // refreshSession returns null data if there's no session yet (user not logged in).
            // Fall back to getUser() which hits the server directly.
            let user = refreshData?.session?.user
            if (!user) {
            ...
            if (refreshError && !user) {
                ...
                // Sync the verified flag in our public users table
                await supabase
                    .from('users')
                ...
                setError('Email not yet verified. Please open the verification email and click the link, then try again.')
            }
        } catch (err) {
