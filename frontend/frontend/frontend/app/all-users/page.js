                            ...
                            {filteredUsers.map((user) => (
                                <div key={user.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
                                    ...
                                    <div className="p-4">
                                        ...
                                        {/* Conditional Info - Show only when following */}
                                        {user.follow_status ? (
                                            <>
                                                {/* Name + badges */}
                                                <Link href={`/workers/${user.id}`}>
                                                    <h3 className="font-bold text-gray-900 mb-1 hover:text-indigo-600 cursor-pointer flex items-center gap-1 flex-wrap">
                                                        {user.first_name} {user.last_name}
                                                        {(user.is_verified || (Array.isArray(user.verification_badges) && user.verification_badges.some(b => b.status === 'approved'))) && <VerifiedBadge size={16} className="flex-shrink-0" />}
                                                        {user.plan_tier === 'premium' && <span className="inline-flex items-center bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-bold px-1 py-0.5 rounded-full flex-shrink-0">💎</span>}
                                                        {user.plan_tier === 'pro' && <span className="inline-flex items-center bg-yellow-400 text-yellow-900 text-xs font-bold px-1 py-0.5 rounded-full flex-shrink-0">⭐</span>}
                                                    </h3>
                                                </Link>
                                                {getReferralTier(user.referral_count ?? 0) && (
                                                    <div className="mb-1"><ReferralBadge tier={getReferralTier(user.referral_count ?? 0)} className="text-xs" /></div>
                                                )}
                                            ...
                                            <>
                                                <h3 className="font-bold text-gray-900 mb-1 text-center flex items-center justify-center gap-1 flex-wrap">
                                                    {user.first_name} {user.last_name}
                                                    {(user.is_verified || (Array.isArray(user.verification_badges) && user.verification_badges.some(b => b.status === 'approved'))) && <VerifiedBadge size={16} className="flex-shrink-0" />}
                                                    {user.plan_tier === 'premium' && <span className="inline-flex items-center bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-bold px-1 py-0.5 rounded-full flex-shrink-0">💎</span>}
                                                    {user.plan_tier === 'pro' && <span className="inline-flex items-center bg-yellow-400 text-yellow-900 text-xs font-bold px-1 py-0.5 rounded-full flex-shrink-0">⭐</span>}
                                                ...
                                                {getReferralTier(user.referral_count ?? 0) && (
                                                    <div className="mb-1 flex justify-center"><ReferralBadge tier={getReferralTier(user.referral_count ?? 0)} className="text-xs" /></div>
                                                )}
