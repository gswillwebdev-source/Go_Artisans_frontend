...
export default function ClientProfilePage() {
    ...
    const handleJobSubmit = async (e) => {
        ...
        // Free-tier: hard limit of 15 new job posts per calendar month
        if (!editingJobId && isFree && !canPostJobs()) {
            setJobFormError(
                `You've reached the ${FREE_JOB_POST_LIMIT} job post limit for this month on the free plan. Upgrade to Pro or Premium for unlimited job posts.`
            )
            return
        }
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            {isFree && (
                                                <p className="text-indigo-200 text-xs mt-3">
                                                    {Math.max(0, FREE_JOB_POST_LIMIT - (jobPostsThisMonth || 0))} of {FREE_JOB_POST_LIMIT} free posts remaining this month.{' '}
                                                    <Link href="/plans" className="underline text-white font-semibold">Upgrade for unlimited →</Link>
                                                </p>
                                            )}
                                        </div>
                                    </div>
