...
export default function JobAlertsPage() {
    ...
    const alertLimit = isFree ? FREE_ALERT_LIMIT : isPro ? PRO_ALERT_LIMIT : Infinity
    const [alerts, setAlerts] = useState([])
    const [loading, setLoading] = useState(true)
    const [showForm, setShowForm] = useState(false)
    const [showAlertUpgradeModal, setShowAlertUpgradeModal] = useState(false)
    const [editingAlert, setEditingAlert] = useState(null)
    const [error, setError] = useState('')
    const [success, setSuccess] = useState('')
    const atAlertLimit = alerts.length >= alertLimit
    ...
    return (
        <div className="profile-page">
            <UpgradeModal
                isOpen={showAlertUpgradeModal}
                onClose={() => setShowAlertUpgradeModal(false)}
                feature="unlimited job alerts"
            />
                        ...
                        <div className="profile-actions">
                            {!isPro || isPro ? (
                                <p className="text-sm text-gray-500 mb-2">
                                    <span className={`font-semibold ${atAlertLimit ? 'text-red-600' : 'text-gray-600'}`}>
                                        {alerts.length}/{alertLimit === Infinity ? '∞' : alertLimit} alerts used
                                    </span>
                                </p>
                            ) : null}
                            <button
                            ...
                            >
                                + {t('createNewJobAlert')}{atAlertLimit ? ' 🔒' : ''}
                            </button>
                        </div>
                    </div>
                </div>
